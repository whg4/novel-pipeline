...
export const PROVIDER_PRESETS: ProviderPreset[] = [
  {
    ...
    helpText: 'DeepSeek 使用 OpenAI-compatible Chat Completions 格式，通常可以直接填 key 调用。',
  ...
  },
  {
    ...
    helpText: '这里接入的是 Anthropic Claude API。Claude Code 本地 CLI 需要后续通过本地 relay 接入，网页不能直接调用本机 CLI。',
    directBrowserSupport: 'proxy-recommended',
  },
  {
    id: 'gemini',
  ...
  },
  {
    ...
    helpText: 'xAI API 基本兼容 OpenAI Chat Completions，模型名称仍可手动调整。',
    directBrowserSupport: 'proxy-recommended',
  },
  {
    id: 'openrouter',
    ...
    modelSuggestions: ['anthropic/claude-3.5-sonnet', 'openai/gpt-4o', 'deepseek/deepseek-chat', 'google/gemini-pro'],
    description: '一个 key 路由多个模型，适合快速比较不同模型效果。',
    helpText: 'OpenRouter 使用 OpenAI-compatible 格式；模型名称通常带供应商前缀，例如 anthropic/claude-3.5-sonnet。',
    directBrowserSupport: 'proxy-recommended',
  },
  {
    id: 'custom-openai',
...
export function normalizeLegacyProvider(provider: string | undefined): LLMProviderId {
  if (provider === 'custom') return 'custom-openai';
  if (PROVIDER_PRESETS.some((preset) => preset.id === provider)) return provider as LLMProviderId;
  return DEFAULT_PROVIDER_ID;
}

export function createConfigForProvider(providerId: LLMProviderId, current?: Partial<APIConfig>): APIConfig {
